...
step secs gstate@(GameState { paused, player = player@(Player {pos, hitbox, fireRate, bullet, health }), keys, lost })=
    ...
    bullets = bulletshit, lost = (health <= 0) }
    else return gstate
...
bulletListUpdate (GameState { player = player@(Player {health}), bullets, enemies}) =
  (map bulletUpdate (filter (\thisBull@(Bullet pos (BulletType speed size bulletDamage )) -> not(outOfBounds pos size) && (isHit player thisBull < 1) ) bullets), totaldam, enemies )
